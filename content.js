...
function stopAnnotationMode() {
  ...
  const json = JSON.stringify(exportAnnotations, null, 2); // Pretty-print for readability
  ...
  if (typeof html2canvas === "function") {
    html2canvas(document.body, {
      ...
      allowTaint: false,
      logging: true,
      scale: 1  // optional: lower for better performance
    })
    .then(canvas => {
    ...
    .catch(err => {
      ...
      alert("Screenshot couldn't be captured due to website restrictions.");
    });
  } else {
    console.warn("html2canvas is not available.");
    alert("Screenshot functionality is unavailable.");
  }
}
